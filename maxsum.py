...
2.3
Given a number n, we can divide it in only three parts n/2, n/3 and n/4 (we will consider only integer part). The task
is to find the maximum sum we can make by dividing number in three parts recursively and summing up them together.
...
def maxsum(n):
    ...
    for i in range(2,n+1):
        maxlist.append(max(maxlist[i//2] + maxlist[i//3] +maxlist[i//4],i))
    return maxlist[n]
